...
public class Generate_game_questions {
	 ...
	 *  specifically:
	 *  
	 *  	Actors: specifies the amount of the top famous actors to be taken from the database.
	 *  	Movies: specifies the amount of the top famous movies to be taken from the database.
	 *  	Music: specifies the amount of the top famous musicians to be taken from the database.
	 *  	Places: specifies the amount of the top famous places to be taken from the database.
	 *  	Sports: specifies the amount of the top famous players to be taken from the database.
	 *  
	 * @throws IOException 
	 * @throws SQLException 
	 ...
	 */
	public static Question[] genrateQuestions(int numOfRounds) 
		...
		for(int i=0; i<numOfRounds; i++){
			...
			if(question_themes[i]==0){
				qst[i]=MakeTheLink.db.Generate_a_question.actors_question(conn);
			}
			
			if(question_themes[i]==1){
				qst[i]=MakeTheLink.db.Generate_a_question.movies_question(conn);
			}
			
			if(question_themes[i]==2){
				qst[i]=MakeTheLink.db.Generate_a_question.music_question(conn);
			}
			
			if(question_themes[i]==3){
				qst[i]=MakeTheLink.db.Generate_a_question.places_question(conn);
			}
			
			if(question_themes[i]==4){
				qst[i]=MakeTheLink.db.Generate_a_question.sports_question(conn, "nba");
			}
			
			if(question_themes[i]==5){
				qst[i]=MakeTheLink.db.Generate_a_question.sports_question(conn, "world_soccer");
			}
			
			if(question_themes[i]==6){
				qst[i]=MakeTheLink.db.Generate_a_question.sports_question(conn, "israeli_soccer");
			}
		}
